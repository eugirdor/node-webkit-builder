...
NwBuilder.prototype.mergeAppFiles = function () {
    function runPromiseOperation(op) {
        return op.fn.apply(op.scope, op.args);
    }
    ...
    this._forEachPlatform(function (name, platform) {
        ...
        if(name === 'osx32' || name === 'osx64') {
            ...
            if(!self.options.macZip) {
                self._files.forEach(function (file) {
                    ...
                    if(file.dest === 'package.json' && platform.platformSpecificManifest){
                        copiedFiles.push({ fn: self.writePlatformSpecificManifest, scope: self, args: [ platform, dest ] });
                    }
                    else {
                        copiedFiles.push({ fn: Utils.copyFile, args: [ file.src, dest, self ] });
                    }
                });
            } else {
                // zip just copy the app.nw
                copiedFiles.push({ fn: Utils.copyFile, args: [
            ...
            copiedFiles.push({ fn: Utils.mergeFiles, args: [
    ...
    return Promise.map(copiedFiles, runPromiseOperation, { concurrency: 256 });
};
